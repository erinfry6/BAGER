...
write.table(TPM_noMT_expressed_log_processed, file= paste("../data/forBAGER/",Sys.Date(),"ProcessedExpData.txt"), sep='\t')

## save list of samples to be included in analysis
write.table(Sample_Info_Second_Elim, file = paste("../data/forBAGER/",Sys.Date(),"IncludedSamples.txt"), sep='\t')

## save expression files by tissue
for (t in tissue){
  write.table(t(TPM_noMT_expressed_log_processed[,grep(t,colnames(TPM_noMT_expressed_log_processed))]) , paste("../data/forBAGER/",t,"_exp",".txt", sep=""), sep='\t', col.names = FALSE)
}
```
