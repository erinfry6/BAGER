...
**This R markdown file will create RNA_seq log transformed gene expression files for Bayesian Ancestral Differential Gene Expression Reconstruction (BADGER) for six primate species and six tissues.**
...
setwd("/Users/lynchlab/Desktop/ErinFry/workflowr/AGER/analysis")
library(dplyr)
library(limma)
library(ggplot2)
library(gplots)
...
TPM_raw<-read.table("../data/Transcript_Abundances31.txt", header=TRUE, sep="\t", na.strings='NA', stringsAsFactors = FALSE) 

## upload gene information
geneinfo<-read.table("../data/mart_export.txt",header=TRUE,sep='\t',na.strings='NA',stringsAsFactors = FALSE)

## load and view the sample information
SampleInfo<-read.table("../data/SampleInformation.txt",sep='\t',header=TRUE)
head(SampleInfo)
...
GtexMedians<-read.table("../data/Gtex_Medians_All.txt",header=T,sep='\t')
...
## Save as tissue expression files for BADGER
```{r save data files}
## first save all processed data
write.table(TPM_noMT_expressed_log_processed, file= paste("../data/forBADGER/",Sys.Date(),"ProcessedExpData.txt", sep=""), sep='\t', quote = FALSE)

## save list of samples to be included in analysis
write.table(Sample_Info_Second_Elim, file = paste("../data/forBADGER/",Sys.Date(),"IncludedSamples.txt", sep=""), sep='\t', quote=FALSE)
...
for (t in tissue){
  ...
  write.table(t(temp_rm) , paste("../data/forBADGER/",t,"_exp",".txt", sep=""), sep='\t', col.names = FALSE, quote=FALSE)
  write.table(temp_rm, paste("../data/forBADGER/",t,"_genesincluded",".txt", sep=""), quote=FALSE, sep='\t')
