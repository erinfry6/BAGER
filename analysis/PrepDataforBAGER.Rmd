...
TPM_noMT<-TPM_noMT[,4:ncol(TPM_noMT_expressed)]

labels <- paste(SampleInfo$Species, SampleInfo$Tissue, sep=" ")

plot_PCs(pca = t(TPM_noMT), firstPC = 1, lastPC = 4, Covars = SampleInfo, points=T, legTorF = F)
...
for (t in 1:length(tissue)){
plotDensities(TPM_noMT_log[,grep(tissue[t], colnames(TPM_noMT_log))],main=paste("log(TPM) Expression Density Curves for", tissue[t], sep=" "), col = pal, legend = "topleft")
  abline(v=log(2))
}

plot_PCs(pca = t(TPM_noMT_log), firstPC = 1, lastPC = 2, Covars = SampleInfo, points=T, legTorF = F)
...
for (t in 1:length(tissue)){
plotDensities(tmm_tpm[,grep(tissue[t], colnames(tmm_tpm))],main=paste("log(TPM)+TMM Expression Density Curves for", tissue[t], sep=" "), col = pal, legend = "topright")
  abline(v=log(2))
}

plot_PCs(pca = t(tmm_tpm), firstPC = 1, lastPC = 2, Covars = SampleInfo, points=T, legTorF = F)

```

** ppa_lv_M_1 is not clustering with the other tissues and will be eliminated**

### Log2 and TMM remove ppa_lv_M_1
Using weighted trimmed mean of M-values algorithm (TMM) to account for differences in the read counts at the extremes of the distribution and calculate the TMM-normalized log2-transformed TPM. 
```{r log2 and TMM}
dge <- DGEList(counts=as.matrix(TPM_noMT[,-(SampleInfo$SampleID=="ppa_lv_M_1")]), genes=rownames(TPM_noMT), group = as.character(t(labels[-which(SampleInfo$SampleID=="ppa_lv_M_1")])))
dge <- calcNormFactors(dge)

tmm_tpm <- cpm(dge, normalized.lib.sizes=TRUE, log=TRUE, prior.count = 0.25)
head(tmm_tpm)

SampleInfo_1elim<-SampleInfo[-which(SampleInfo$SampleID=="ppa_lv_M_1"),]

boxplot_library_size <- ggplot(dge$samples, aes(x = SampleInfo_1elim$Tissue, y = dge$samples$lib.size, fill = SampleInfo_1elim$Species)) + geom_boxplot()
...
plot_PCs(pca = t(tmm_tpm), firstPC = 1, lastPC = 2, Covars = SampleInfo_1elim, points=T, legTorF = F)
...
for (t in tissue){
  ...
  print(colnames(TPM_noMT_expressed_log)[grep(t, colnames(TPM_noMT_expressed_log))][which(abnormQ<0.023 | abnormQ>0.977)])
...
SampleInfo$Failures[which(SampleInfo$SampleID %in% abnorm[1,])]<-SampleInfo$Failures[which(SampleInfo$SampleID %in% abnorm[1,])]+3
