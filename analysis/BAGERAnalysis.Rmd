...
# Setup
## Define paths and load libraries, BAGER results, and gene information
...
## Define functions to visualize expression divergence
...
viewPPDs<-function(genelist,lastit=1000,nbins=100){
  ...
  hist(gene$Est.AncHomo...1,
       ...
       col="#00b5f5", breaks=bins, xlim=c(minimum,maximum))
...
## Load and Examine the Results from BAGER Analysis
...
## Select reconstructed and expressed genes
Genes that successfully reconstructed will have similar posterior variations between the two ancestral node reconstructions. Each reconstructions difference in standard deviation between the two ancestral nodes has been calculated. Only genes with a foldSD less than 4 will be included, as they have successfully reconstructed.

```{r reconstruction success, warnings=FALSE, echo =TRUE}
...
**I have chosen to use the Bayesian Posterior Probability of Divergence to identify genes with different expression levels in ancHuman and ancHuman-Chimpanzee nodes. The BPPD is the proportion of iterations for which the change along the human lineage is in concordance, or to one side of zero. Genes with a BPPD of greater than 0.90 are considered genes with expression shifts in the human lineage.**

```{r transcriptome qualities of converged reconstructions, echo=TRUE, warning=FALSE}
...
viewDifference(shiftgenes$gene_number[1:10])
...
## Number of genes in each BPPD category
```{r gene count in BPPD category, warning=FALSE, echo=FALSE}
...
## Interactive Reconstructed Transcriptome Summary Plot
...
To visualize expression shifts, one can view the posterior probability divergences of the ancHuman (teal) and ancHC (gray).
...
# BAGER method success

## Number of genes identified to have an expression shift

**Using a BPPD cutoff of 90%, `r nrow(shiftgenes)` (of `r nrow(reconsgenes)` reconstructed and expressed genes) are identified to have expression shifts in the human lineage. This is a `r nrow(shiftgenes)/4` fold increase in genes identified to have shifted expression in the human frontal cortex from Brawand et al 2011.**

## Heatmap of expressed genes compared to all

These heat maps are generated using the expression data in all sampled primate frontal cortices. The first, of all reconstructed genes, does not elucidate any obvious patterns. The second, of all genes with expression shifts, shows clusters of genes with specific patterns. The most notable of which is either increased or decreased expression in humans compared to all other species.
