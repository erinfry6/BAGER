...
shiftanalysis<-function(reconsdf, BPPDcutoff=0.9, minexpression= expcut, lineage.df, histogram=FALSE, write.results=FALSE){
...
createScatterPlot<-function(reconsdf, listnumber, minexpression= expcut, lineage.df, print.BPPDs=FALSE, interactive=FALSE, save.scatter=FALSE) {
  reconsdf<-reconsdf[[listnumber]]  # first choose the dataframe from the list of dataframes
  reconsdf$BPPDLevel <- cut(reconsdf[,grep(colnames(reconsdf), pattern = "BPPD")], c(0,0.90,0.95,0.975,1.1)) # break BPPD into categories

  if(print.BPPDs==TRUE){
    BPPDcats<-reconsdf %>% 
    group_by(BPPDLevel) %>%
    summarise(no_genes=length(BPPDLevel))
    print(BPPDcats) }  # view the number of genes in each category
  ...
  ## plot using ggplot
p<-ggplot(data=reconsdf, mapping=aes(
  x=eval(as.symbol(colnames(reconsdf)[grep(colnames(reconsdf), pattern = "reconAnc")])),
  y=eval(as.symbol(colnames(reconsdf)[grep(colnames(reconsdf), pattern = "reconDesc")])), 
  text=hgnc_symbol)) + 
  theme_bw() +
  theme(plot.title= element_text(size=15, face="bold"), axis.title.x=element_text(size=20),axis.title.y=element_text(size=20)) +
  ggtitle(paste("Ancestral Transcriptomes of ", lineage.df[listnumber,1], " and ", lineage.df[listnumber,2],sep="")) +
  labs( x=paste(lineage.df[listnumber,1]," log(TPM)", sep=""), 
        y=paste(lineage.df[listnumber,2]," log(TPM)", sep=""), face="bold", size=20) +
  ...
  + theme (legend.text = reconsdf[,grep(colnames(reconsdf), pattern = "BPPD")])

if (interactive==TRUE){
  print(suppressWarnings(ggplotly(p)))
} else {
  suppressWarnings(print(p))
}

if (save.scatter==TRUE){
  ggsave(paste(pathResults,Sys.Date(),"Scatter.pdf",sep="")) }
}
...
p<-ggplot(data=test, mapping=aes(
  ...
  theme_bw() +
  theme(plot.title= element_text(size=15, face="bold"), axis.title.x=element_text(size=20),axis.title.y=element_text(size=20)) +
  ggtitle(paste("Ancestral Transcriptomes of ", lineages.to.test[listnumber,1], " and ", lineages.to.test[listnumber,2],sep="")) +
  labs( x=paste(lineages.to.test[listnumber,1]," log(TPM)", sep=""), 
        y=paste(lineages.to.test[listnumber,2]," log(TPM)", sep=""), face="bold", size=20) +
  geom_point(data=df_layer_1, colour="#ffffcc", alpha=1) +
  geom_point(data=df_layer_90, colour="#a1dab4", alpha=1) +
  geom_point(data=df_layer_95, colour="#41b6c4", alpha=1) +
  geom_point(data=df_layer_99, colour="#0c2c84", alpha=1) +
  ...
  theme (legend.text = test[,grep(colnames(test), pattern = "BPPD")])
```
...
for (l in 1:nrow(lineages.to.test)){
    ...
    temp<-filter(Summary, eval(as.symbol(paste(lineage,"foldSD",sep="_"))) <cutoffSD,  .data = eval(as.symbol(paste(lineage,"reconAnc",sep="_")))>expcut | eval(as.symbol(paste(lineage,"reconDesc",sep="_")))>expcut)
...
for (l in 1:nrow(lineages.to.test)){
    ...
    createScatterPlot(reconsgenes, listnumber = l, lineage.df = lineages.to.test, interactive = TRUE)
}
...
createScatterPlot(reconsgenes, listnumber = 8, lineage.df = lineages.to.test, interactive =TRUE, print.BPPDs = TRUE )
