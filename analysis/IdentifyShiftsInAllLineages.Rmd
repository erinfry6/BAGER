...
shiftanalysis<-function(reconsdf=reconsgenes, listnumber, lineages.df=lineages.to.test, histogram=FALSE, write.results=FALSE){
  ...
  write.table(reconsdf,paste(pathResults,Sys.Date(),lineage,"Reconstructed.txt", sep=""),sep='\t')
  ...
  return(c(nrow(reconsdf[[listnumber]]),nrow(shiftgenes), num.recons[listnumber],length(listcsv)))
}
...
## uploaded saved file 
Summary<-read.table(paste(pathResults,"2017-07-30BAGERSummary.txt", sep=""),sep='\t', header=T)
...
Genes that succesfully reconstructed will have similar posterior variations in the two ancestral node reconstructions. For each lineage, select only genes with fold standard deviations between the ancestral and descendant node of less than 4. Save these gene lists in the list of dataframes, reconsgenes.
...
```{r for saving number of shifts in each lineage, eval=FALSE, include=FALSE}
...
WebGestaltR(enrichMethod = "ORA", organism = "hsapiens", enrichDatabase = enrichdb,
            ...
            outputDirectory=paste(pathResults,"Enrichments/",sep=""),projectName=paste(Sys.Date(),lineages.to.test[l,1],lineages.to.test[l,2],enrichdb,sep="-"),
...
```{r examine qualities of genes identified to have shifts in the original data, eval=FALSE, cache=TRUE}
...
converged_expression<-prepforheatmap(TPM_log[as.numeric(test),])
...
```{r ancrecon as mean exploration, eval=FALSE,warning=FALSE}
## human mean
